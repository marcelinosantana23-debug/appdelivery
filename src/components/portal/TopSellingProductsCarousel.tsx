        ...
        {products.map((item, index) => {
          ...
          return (
            <div
              ...
              className={`group relative flex w-[240px] sm:w-[260px] shrink-0 snap-start flex-col overflow-hidden rounded-2xl border bg-white dark:bg-slate-900 shadow-sm transition-all duration-300 hover:shadow-lg hover:-translate-y-1 cursor-pointer ${
                isLeader
            ...
            >
              ...
              <div className="relative aspect-4/3 w-full overflow-hidden bg-slate-950">
                {item.image ? (
                  ...
                  <div className="flex h-full w-full items-center justify-center bg-gradient-to-tr from-amber-600/40 to-red-600/40 text-4xl">
                    🍔
                  </div>
                )}
                ...
                <div className="absolute top-2.5 left-2.5 z-10">
                  <div
                    className={`flex items-center gap-1 rounded-full px-2.5 py-0.5 text-[11px] font-black shadow-md backdrop-blur-md ${
                      isLeader
                ...
                {item.totalSold > 0 && (
                  <div className="absolute top-2.5 right-2.5 z-10">
                    <span className="inline-flex items-center gap-1 rounded-full bg-black/70 px-2 py-0.5 text-[10px] font-bold text-white backdrop-blur-md border border-white/10">
                      <span>🔥</span>
                      <span>{item.totalSold} {item.totalSold === 1 ? "venda" : "vendas"}</span>
                    </span>
                  </div>
                )}

                {/* Loja que vende o lanche (sobreposta no rodapé da imagem) */}
                <div className="absolute bottom-2 left-2.5 right-2.5 z-10 flex items-center gap-1.5 rounded-lg bg-black/60 px-2 py-1 backdrop-blur-md border border-white/15">
                  <div className="flex h-4 w-4 shrink-0 items-center justify-center rounded-md bg-white/20 text-[10px]">
                    <Store className="h-2.5 w-2.5 text-amber-300" />
                  </div>
              ...
              <div className="flex flex-1 flex-col justify-between p-3.5">
                <div>
                  ...
                  {item.description ? (
                    ...
                    <p className="mt-1 text-xs text-gray-400 dark:text-gray-500 italic">
                      Disponível para pedido imediato
                    </p>
                  )}
                </div>
                ...
                <div className="mt-3 border-t border-gray-100 dark:border-slate-800 pt-2.5 flex items-center justify-between gap-2">
                  <div>
                    <span className="block text-[10px] uppercase font-bold text-gray-400 dark:text-gray-500">
                      Preço
                    </span>
                  ...
                  <button
                    ...
                    className="flex items-center gap-1 rounded-xl bg-primary hover:bg-primary-dark px-3 py-1.5 text-xs font-bold text-white shadow-xs transition active:scale-95 cursor-pointer"
                  >
